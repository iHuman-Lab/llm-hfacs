...
def precondition_rf_gpt(config):
    """RF trained on ground truth features (500 lora-train rows), tested on GPT-4o mini llm_* features (200 lora-test rows)."""
    ...
    X_train = train_df[aligned_bare].astype(float)
    y_train = train_df["category"]

    X_test = test_df[llm_cols].astype(float)
    X_test.columns = (
        aligned_bare  # align with ground truth feature names used in training
    )
    y_test = test_df["category"]

    print(
        f"\n[RF-GPT-Preconditions] Train: {len(X_train)} rows (ground truth) | Test: {len(X_test)} rows (GPT-4o mini llm_*)"
    )
    print(f"Features: {len(aligned_bare)} precondition columns")
    ...
    save_predictions(y_test, y_pred, out_dir, "rf_gpt_preconditions_predictions.csv")
