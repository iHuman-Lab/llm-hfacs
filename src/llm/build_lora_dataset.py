...
OUTPUT_DIR = "data/processed/lora"
PROMPT_PATH = "src/llm/prompts/ASRS/ASRS_extract_and_classify_lora.yaml"


def _present(df: pd.DataFrame, cols: list[str]) -> pd.Series:
    valid = [c for c in cols if c in df.columns]
    if not valid:
        return pd.Series(False, index=df.index)
    return (df[valid].apply(pd.to_numeric, errors="coerce").fillna(0) > 0).any(axis=1)
...
def _build_label(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    for cat, cols in config["hfacs_categories"].items():
        if cat in skip:
            continue
        labels[cat] = _present(df, cols).map({True: "Yes", False: "No"})
